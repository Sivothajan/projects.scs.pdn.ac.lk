...
const LOCAL_DATA_PATH = path.resolve(process.cwd(), "./api/data/v2");
...
const fetchData = async (reqPath, filePath, jsonName, githubUrl) => {
  try {
    ...
    if (reqPath.startsWith("/api/v2/course/")) {
    ...
    } else if (reqPath.startsWith("/api/v2/projects/cc/")) {
      // Related projects by course code
      localPath = LOCAL_COMMON_PATH + `/${filePath.toLowerCase()}`;
    } else if (reqPath.startsWith("/api/v2/projects/id/")) {
      // Project by ID
      localPath = LOCAL_PROJECT_PATH + `/${filePath.toLowerCase()}`;
    } else if (reqPath.startsWith("/api/v2/student/")) {
      // Student by ID
...
v2Router.get("/api/v2/courses/:courseCode", async (req, res) => {
  try {
...
// Get all courses
v2Router.get("/api/v2/courses", async (req, res) => {
  try {
...
v2Router.get("/api/v2/instructor/:instructorUsername", async (req, res) => {
  try {
...
v2Router.get("/api/v2/projects/cc/:courseCode", async (req, res) => {
  const { courseCode } = req.params;
  if (!courseCode || typeof courseCode !== "string") {
    return res.status(400).json({ error: "Invalid course code" });
  }
...
v2Router.get("/api/v2/projects/id/:projectId", async (req, res) => {
  try {
...
v2Router.get("/api/v2/student/:studentId", async (req, res) => {
  try {
